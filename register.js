...
function minus(i){cart[i].qty--;if(cart[i].qty<1)cart.splice(i,1);save();render()}
async function read(raw){try{const p=await decodeQR(raw);add(p);status.textContent=`追加：${p.name} ${yen(p.price)}`;navigator.vibrate?.(60)}catch(e){status.textContent=e.message}}
async function scan(){
 ...
 if(video.videoWidth){
  ...
  if("BarcodeDetector" in window){
   if(!window._detector)window._detector=new BarcodeDetector({formats:["qr_code"]});
   const found=await window._detector.detect(c);
...
async function startCam(){
 try{
  if(!navigator.mediaDevices?.getUserMedia)throw Error("カメラ非対応です");
 ...
 }catch(e){status.textContent=e.message+"。カメラ許可を確認してください"}
}
function stopCam(){scanning=false;if(stream){stream.getTracks().forEach(t=>t.stop());stream=null}video.srcObject=null;status.textContent="停止しました"}
function payNow(){
...
function showHistory(){
 const h=JSON.parse(localStorage.getItem("bpos_history")||"[]");
 history.innerHTML=h.length?h.slice().reverse().map(x=>`<div class="history-line"><span>#${String(x.id).padStart(4,"0")}　${x.time}</span><b>${yen(x.total)}</b></div>`).join(""):"履歴はありません";
 historyModal.hidden=false;
}
start.onclick=startCam;stop.onclick=stopCam;pay.onclick=payNow;clear.onclick=()=>{cart=[];save();render()};
historyBtn.onclick=showHistory;historyClose.onclick=()=>historyModal.hidden=true;
historyClear.onclick=()=>{if(confirm("履歴を全消去しますか？")){localStorage.removeItem("bpos_history");showHistory()}};
historyModal.addEventListener("click",e=>{if(e.target===historyModal)historyModal.hidden=true});render();
